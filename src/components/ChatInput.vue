    ...
    <div class="chat-input">
        ...
        <div class="chat-input-wrapper">
            <textarea v-model="messageInput" @keydown.enter="handleEnterKey" @input="adjustTextareaHeight"
                placeholder="输入消息，按Enter发送，Shift+Enter换行" ref="messageTextarea" rows="1"
                :disabled="isResponding"></textarea>
            <div class="chat-toolbar">
                <select class="model-selector" v-model="selectedModel" :disabled="isResponding">
                    <option v-for="model in modelConfigs" :key="model.name" :value="model">{{ model.name || '未命名模型' }}
                    </option>
                </select>
